...
- **The wallet falls back to the default credential endpoint when an issuer publishes an empty one.** An empty `credential_endpoint` string was taken as given and produced an empty request URL, where a missing one already fell back to `<issuer>/credential`. Both are treated the same now.
