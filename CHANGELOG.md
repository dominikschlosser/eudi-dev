...
- **The wallet tells the Verifier why it refused.** A malformed request, a HAIP violation, or a query nothing matched was reported only to the local caller, leaving a conformant Verifier waiting on its `response_uri` until it timed out with no idea why. Every refusal goes back over the request's Response Mode as an OpenID4VP 1.0 §8.5 Authorization Error Response now, with the code the section calls for (`access_denied` for nothing to present, `vp_formats_not_supported` for a query naming only formats the wallet cannot present, `invalid_request_uri_method` for a `request_uri_method` that is neither `get` nor `post`, `invalid_transaction_data` for transaction data the wallet does not support, `invalid_request` otherwise). The local API keeps its own answer
