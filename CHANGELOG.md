...
- **The demo seeds both PIDs.** Its protected baseline is now four credentials, the country-independent PID and the German one in each format. The demo verifier can ask for either: its PID request names `urn:eudi:pid:1`, which both answer, and its new German PID request names `urn:eudi:pid:de:1` and a national attribute, which only the German credential can answer
