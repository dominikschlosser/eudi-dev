...
- **Credential cards carry the appearance the issuer declared.** The §12.2.4 display properties of the issued configuration (name, description, logo, background and text color, background image) are stored with the credential and rendered on its card, in the presentation consent dialog and in the credential-offer preview: the display name leads, the technical type stays dimmed beside it (a DCQL query matches on the vct, so a diagnostic wallet keeps it visible). The background image and background color fill the card face (one background slot, the way a wallet card has): the image over the color as its base, or a solid color face, or a plain tile with a monogram when the issuer declared neither. The declared text color paints the name and the format badge over the face, and the color lives only in the face (it never tints the list row). The card lays itself out from the list width (a dense row with the face as a small thumbnail on a wide list, a wallet-style card with the face as a full-width hero on a phone). Beside the appearance the card carries its own honest trust state (status, validity, a self-consistent signature that is never shown as issuer trust, holder binding), the issuer as the format names it, and how long ago it was issued with the short id that tells two same-type instances apart. The presentation consent dialog and the credential-offer preview render the same card. The consent dialog names who is asking (the verifier client_id or the issuer, with a self-asserted client_name when the request carries one) and, for a presentation, whether the request object was signed and self-consistent or not authenticated at all, and it lists the exact fields being disclosed as toggles that can withhold even a requested claim. Images are fetched through the policed client (up to 4MB), downscaled to card size when they are larger than the 256KB cache cap, checked to be images and bounded to 32 megapixels before decode, and embedded as data URIs, so rendering a card never calls the issuer again. Every fetch, at issuance and in the offer preview alike, is held to the same internal-address policy as the rest of the issuance, since on a shared public demo anyone can issue and the offer's metadata is attacker-controlled. A color outside the CSS Color Module Level 3 value space is dropped with a warning, and a pair below 3:1 WCAG contrast is warned about for the wallets that would paint it as declared (`credential_display_invalid`, `credential_display_low_contrast`, `credential_display_image_rejected`). Style findings are warnings in every mode: cosmetics never fail an issuance. A deferred credential keeps its declared appearance until the poller collects it, and a refresh carries it over. The first display entry is used whatever its locale, the rule the consent dialog already followed
- **The demo issuer and the generated credentials wear the eudi-dev look.** The demo issuer declares the eudi-dev logo for itself and its event ticket (served at `/issuer/logo.svg`), the ticket comes in the brand blue, and the auto-generated PID credentials carry the logo and the same colors, so a fresh wallet shows styled cards out of the box. The generated German PID (`urn:eudi:pid:de:1`) is told apart from the country-independent one by its own name and the public German ID card specimen (the Personalausweis "MUSTER") as its card art, the same reference document the EUDI deployments use
