...
- **A credential listing no longer ships every card's art inline.** The display logo and background images were embedded in `/api/credentials` as base64 data URIs, so a wallet holding a dozen styled credentials returned megabytes and the cards appeared only once the whole list had loaded. The listing now references each image by URL (`/api/credentials/{id}/display/{logo|background}`), served on its own and cached hard (immutable, with an ETag), so cards render at once and the browser fetches the art in parallel.
