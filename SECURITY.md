...
- **Only the EUDI stack is supported**: a mechanism the EUDI ARF and the specifications it references do not define is never used to establish anything here, only recognised and reported ([ADR-0013](docs/adr/0013-only-the-eudi-stack-is-supported.md)). DID-based `client_id` values are parsed but not resolved against any DID registry, an issuer or Status Issuer key named by a DID is reported rather than resolved, and an `openid_federation:` client identifier is refused.
...
- **Key attestations claim what they are asked to claim**: The wallet issues its own key attestations (OpenID4VCI Appendix D) and, by default, states the `key_storage` and `user_authentication` levels the issuer requires, `iso_18045_high` included. Its keys live in files, so no level is true. The claims exist so that issuers gating on a level can be tested. `--key-attestation-level none` claims nothing, `--key-attestation-level <level>` claims that level for both, the Conformance panel changes it at runtime on a local wallet, and every attestation carrying a claim is marked in the activity log.
