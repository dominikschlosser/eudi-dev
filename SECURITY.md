...
- **No pre-registered verifier or issuer trust**: A signed request object's `x5c` chain, and a signed Credential Issuer Metadata document, are checked for a valid signature and for an internally consistent chain, but they are *not* anchored to a pre-registered verifier or issuer CA or trust list. A test wallet cannot know those CAs in advance, so there is nothing to anchor to. An attacker who signs a request object (or issuer metadata) with their own certificate and sets the matching `x509_hash` / `sub` passes these checks. Treat a green request-object or issuer-metadata signature as "well-formed and self-consistent", not as "from a trusted party".
