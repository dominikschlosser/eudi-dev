...
func generatedPIDDisplay(vct string) *CredentialDisplay {
	d := &CredentialDisplay{
		Name:            "EUDI PID",
		Description:     "Generated by eudi-dev for development and testing",
		Locale:          "en-US",
		LogoURI:         LogoDataURI(),
...
func (w *Wallet) generateDefaultCredentials(claimOverrides map[string]any, vct string, dropExisting bool) error {
	...
	if !keptSD {
		...
		w.rememberDisplay(sdCred, generatedPIDDisplay(vct))
	...
	if !keptMDoc {
		...
		w.rememberDisplay(mdocCred, generatedPIDDisplay(vct))
