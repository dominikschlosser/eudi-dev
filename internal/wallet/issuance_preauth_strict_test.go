...
// TestProcessCredentialOffer_PreAuthAttestsAfterInvalidClient covers an issuer
// that requires client attestation without advertising it anywhere, which the
// Animo playground does: its authorization server metadata carries no
// token_endpoint_auth_methods_supported at all, so the only signal is the
// refusal itself.
func TestProcessCredentialOffer_PreAuthAttestsAfterInvalidClient(t *testing.T) {
	w := generateTestWallet(t)
	credRaw := generateTestCredential(t, w)
	...
	srv := httptest.NewServer(http.HandlerFunc(func(rw http.ResponseWriter, r *http.Request) {
		...
		case strings.HasSuffix(r.URL.Path, "/.well-known/oauth-authorization-server"):
			// Deliberately silent about client authentication.
			json.NewEncoder(rw).Encode(map[string]any{
				"issuer":         serverURL,
				"token_endpoint": serverURL + "/token",
			})
	...
	if tokenAttempts != 2 {
		t.Errorf("token endpoint saw %d attempts, want 2 (one plain, one attested)", tokenAttempts)
	}
}
