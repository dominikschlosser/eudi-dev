...
func (s *Server) handleBrowserPresentationAPI(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		// A request that fails validation gets no protocol response: §8.5
...
// Error Response through the Digital Credentials API, where the response and
// the answer to the local caller are the same thing: Appendix A.4 says "a
// protocol error generated by the Wallet will still result in a fulfilled
// promise". Only an error response that cannot be built at all (dc_api.jwt
// with no usable encryption key) becomes a plain failure.
