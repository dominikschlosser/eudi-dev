...
// writeBrowserAuthorizationError returns an OpenID4VP 1.0 §8.5 Authorization
// Error Response through the Digital Credentials API.
//
// Over the DC API the response and the answer to the local caller are the same
// thing: Appendix A.4 says "Protocol error responses are returned as an object
// within the data property" and that "a protocol error generated by the Wallet
// will still result in a fulfilled promise", so a refusal is a fulfilled call
// carrying the error object, not an HTTP failure. Only when the error response
// cannot be built at all (a dc_api.jwt request with no usable encryption key)
// is there nothing to hand back, and the caller gets the plain failure instead.
