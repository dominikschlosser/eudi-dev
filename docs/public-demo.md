...
4. It enforces the EUDI profile. `--mode strict` makes request validation findings fatal instead of warnings (bad certificate chain, broken request object signature, missing nonce, unknown `client_id` prefix), and `--haip` requires HAIP 1.0 on incoming presentations: a signed request object, an `x509_hash:` / `x509_san_dns:` / `web-origin:` client id, `direct_post.jwt` or `dc_api.jwt`, DCQL, and ES256. **A verifier that is not HAIP-compliant gets HTTP 400.** Both are overridable (`--mode debug`, `--haip=false`) for a self-hosted demo that would rather be permissive, and a single request can opt out with `{"haip": false}` on `POST /api/presentations`. Issuance is not enforced yet. The built-in demo verifier is itself HAIP-compliant, so the demo works out of the box under these rules. The active level is visible under **Conformance** in the wallet UI header.
