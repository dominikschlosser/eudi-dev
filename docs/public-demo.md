...
A hosted wallet cannot open a browser of its own, so it hands the authorization URL to the tab that started the flow (an `authorize` event on `/api/requests/stream`) and that tab navigates. The issuer redirects back to the wallet's `/callback`, which resumes the flow already in progress and returns the visitor to the wallet UI. A wallet running on the user's own machine still opens a local browser, unchanged.
