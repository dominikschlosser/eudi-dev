...
The verifier page asks for either. Its PID request names `urn:eudi:pid:1`, which both credentials answer (that is [credential type inheritance](wallet.md#credential-type-inheritance) at work, and the wallet log names the German PID as a candidate), and the wallet presents one of them. Its German PID request (`#request-pid-de`, next to `#request-pid` and `#request-ticket`) names `urn:eudi:pid:de:1`, which only a credential of that type answers, the other half of the rule: inheritance runs one way. Nothing about that request is German. `POST /verifier/api/requests` takes the type as `vct` (any type in `urn:eudi:pid:`, so `urn:eudi:pid:fr:1` works the same), and a request naming one is SD-JWT VC only, because a national PID has no doctype of its own to ask for.
