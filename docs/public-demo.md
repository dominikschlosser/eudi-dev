...
1. It implies `--pid`, so the server runs headless with a known credential baseline. Browser flows keep interactive consent (visitors approve offer and authorize links in the wallet UI). API submissions auto-accept, so the demo stays a reliable counterparty for external issuers, verifiers, and CLI clients.
...
The demo sets no cookies. It keeps a theme preference and a dismissed-banner flag in the browser's `localStorage` (no personal data, no tracking, no third parties). Under the ePrivacy Directive that is strictly-functional device storage and needs no consent, but you may still mention it in your imprint or privacy notice.
