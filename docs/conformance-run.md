...
- `OIDF_REQUEST_TIMEOUT`: seconds the wrapper waits on the suite's own API. Defaults to `120`. A poll that gives up is retried, and each retried submission makes the wallet fetch the same `request_uri` again, which fails the module on `RequestUriFetchedMoreThanOnce`
