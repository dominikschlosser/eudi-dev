...
- VCI Final SD-JWT and mDoc wallet plans pass in strict mode, including the batch credential issuance module (the wallet sends multiple distinct proof keys and matches the reordered credentials by binding key). The mdoc plans use the key attestation configuration, where every proof carries the one attestation covering all batch keys (HAIP §4.5.1).
