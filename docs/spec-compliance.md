...
| Batch credential issuance | Implemented | Requests one proof per copy and imports whatever the issuer returns, one credential per proof key (§8.3), even fewer than requested and none bound to the holder key. Each copy records the key it names so its key binding stays signable |
