...
| Batch credential issuance | Implemented | Requests one key per copy (one proof each, or under a required key attestation one proof whose attestation names every key, Appendix F.1) and imports whatever the issuer returns, one credential per key (§8.3), even fewer than requested and none bound to the holder key. Each copy records the key it names so its key binding stays signable |
