...
Localhost is not a boundary against the browser, because every page a developer visits can reach localhost too. So `/api/` refuses requests carrying a cross-origin `Origin` (`internal/httpsec/origin.go`), which costs the intended callers nothing because a CLI or curl sends no `Origin` at all. `/api/dc-api` is exempt by name: the Digital Credentials API is cross-origin by contract, since a verifier's page invokes it from that page's own origin. That endpoint relies on the origin the request arrived with (which is how OpenID4VP over the Digital Credentials API identifies an unsigned request's caller) plus the consent dialog. The protocol endpoints (`/authorize`, `/credential-offer`, `/callback`) stay open to any origin.
