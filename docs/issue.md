...
When no `--claims` are provided, a minimal set of PID-like claims is used (given_name, family_name, birthdate). With `--pid`, the full PID claim set of the requested type is generated: fourteen top-level claims for SD-JWT (including the nested `address` and `place_of_birth` objects) and eighteen mdoc elements, following the [EUDI PID Rulebook](https://github.com/eu-digital-identity-wallet/eudi-doc-attestation-rulebooks-catalog/blob/main/rulebooks/pid/pid-rulebook.md) attribute for attribute. It carries no age attributes, which that rulebook removed in version 1.1 following CIR 2024/2977.

`--vct urn:eudi:pid:de:1` selects the German PID instead, which follows the German PID Rulebook: national attributes added, claims the German eID does not supply left out, and twenty-four mdoc elements across the two namespaces that rulebook defines. A credential from this tool and one from the German PID provider are interchangeable. Both PID claim sets come from the pre-defined `pid-sdjwt`, `pid-mdoc`, `german-pid-sdjwt` and `german-pid-mdoc` credential templates, so a user template saved under one of those names changes what `--pid` issues.
