...
When no `--claims` are provided, a minimal set of PID-like claims is used (given_name, family_name, birthdate). With `--pid`, the full German PID claim set is generated: fourteen top-level claims for SD-JWT (including the nested `address`, `place_of_birth` and `age_equal_or_over` objects) and twenty-four mdoc elements across the two namespaces the German PID Rulebook defines. These follow that rulebook claim for claim, so a credential from this tool and one from the German PID provider are interchangeable. The PID claim sets come from the pre-defined `german-pid-sdjwt` and `german-pid-mdoc` credential templates, so a user template saved under one of those names changes what `--pid` issues.
