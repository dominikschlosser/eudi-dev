...
An imported credential that names a holder key in its `cnf` (SD-JWT) or its MSO `deviceKey` (mdoc) can only be presented by the wallet holding that key. The wallet signs key binding with its own holder key, so a credential issued to another wallet is stored, listed and decoded but never presented successfully (a verifier refuses the key binding signature). The import reports this in the activity log, `wallet import` and `wallet scan` print a warning, the credential card carries a **No holder key** badge, and the credential summary carries `key_binding_not_held`.
